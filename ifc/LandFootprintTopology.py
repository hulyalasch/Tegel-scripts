...
parser = argparse.ArgumentParser(description='Tool to produce discover topology '
                                             ...
                                             'and 2D footprint of building data and materialise'
                                             'them in RDF triples.')
parser.add_argument('-lf', help='Land parcel file path in RDF with GML geometries. ', required=True)
parser.add_argument('-bf', help='2D building footprint file path in RDF with GML geometries. ', required=True)
...
footprint_g = Graph()
footprint_g.parse(args.bf)
topo_g = Graph()

land_geo = map_land_geometries(land_g, args.lf)
footprint_geo = map_footprint_geometries(footprint_g, args.bf)

for footprint_key, footprint_val in footprint_geo.items():
  for land_key, land_val in land_geo.items():
    ...
    topo_g = evaluate_topology(footprint_val, land_val_2d, footprint_key, land_key, topo_g)
    topo_g = evaluate_topology(land_val_2d, footprint_val, land_key, footprint_key, topo_g)
  if footprint_val.GetGeometryCount() > 0:
    for i in range(0, footprint_val.GetGeometryCount()):
      g = footprint_val.GetGeometryRef(i)
      for land_key, land_val_2d in land_geo.items():
        topo_g = evaluate_topology(g, land_val_2d, footprint_key, land_key, topo_g)
        topo_g = evaluate_topology(land_val_2d, g, land_key, footprint_key, topo_g)
